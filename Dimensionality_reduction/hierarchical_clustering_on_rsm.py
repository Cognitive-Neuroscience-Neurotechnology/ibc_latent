...
def load_rsm(file_path):
    """Load an RSM from a .npy file."""
    print(f"Loading RSM from {file_path}")
...
def main():
    ...
    subjects = [f.split('_')[-1].replace('.npy', '') for f in os.listdir(topographic_alignment_RSM_dir) if f.startswith('topographic_alignment_rsm_')]
