...
Stage 2 consumes those saved files to compute flexibility metrics and plots.
...
def load_parcel_set(
    ...
    parcel_ids = [int(pid) for pid in unique_parcels]
    parcel_masks = {int(pid): (parcellation_cortex == int(pid)) for pid in unique_parcels}
    return cortical_indices, parcel_ids, parcel_name_map, parcel_masks
...
def compute_subject(subject: str, args: argparse.Namespace, output_dir: Path) -> None:
    ...
    for ses_path in sessions:
        ...
        for func_path in func_files:
            ...
            motion_path = Path(ses_path) / "postfmriprep" / "regressors" / motion_fname
