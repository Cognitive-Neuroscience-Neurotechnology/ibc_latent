...
%% ---- Step 1: Temporal Concatenation of fMRI data from all resting-state sessions.
...
% Find all resting-state cleaned dtseries files
session_dirs = dir([surface_dir '/ses-*']);
session_dirs = session_dirs([session_dirs.isdir]);

files = {};
for s = 1:length(session_dirs)
    ses_name = session_dirs(s).name;
    % Check both AP and PA directions - only RestingState files end with _cleaned.dtseries.nii
    ap_file = fullfile(surface_dir, ses_name, 'postfmriprep', 'GLM', ...
        ['sub-' Subject '_' ses_name '_task-RestingState_dir-ap_cleaned.dtseries.nii']);
    pa_file = fullfile(surface_dir, ses_name, 'postfmriprep', 'GLM', ...
        ['sub-' Subject '_' ses_name '_task-RestingState_dir-pa_cleaned.dtseries.nii']);
...
if isempty(files)
    error('No resting-state dtseries files found for subject %s', Subject);
end

disp(['Found ' num2str(length(files)) ' resting-state files to concatenate.']);
...
if isempty(ConcatenatedData)
    error('No usable input CIFTI files were found or readable. Check paths and filenames.');
end
