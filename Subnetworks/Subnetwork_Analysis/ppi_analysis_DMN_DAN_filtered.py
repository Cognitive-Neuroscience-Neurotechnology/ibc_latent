...
Step 1/3 -> PPI per Contrast on Individual-level
Filtered for only valuable conditions
"""
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../../public_analysis'))
...
base_dir = os.path.join(working_dir, 'fmriprep_out')
subnetwork_dir = os.path.join(working_dir, 'subnetworks', 'infomap', f'sub-{subject}')
...
# -------------------------------------------
# Check if subnetworks are cortex-only or include subcortex
cortex_size = 59412  # Standard fsLR cortical vertices
subnet1_cortex = subnet1_mask[:cortex_size].sum()
subnet1_subcortex = subnet1_mask[cortex_size:].sum()
subnet2_cortex = subnet2_mask[:cortex_size].sum()
subnet2_subcortex = subnet2_mask[cortex_size:].sum()

print(f"\nSubnetwork composition:")
print(f"  FPN1: {subnet1_cortex} cortical, {subnet1_subcortex} subcortical vertices")
print(f"  FPN2: {subnet2_cortex} cortical, {subnet2_subcortex} subcortical vertices")
print(f"  Total grayordinates in functional data: {len(label_vec)}")
# -------------------------------------------
...
# Print chosen labels for verification
print(f"\nChosen DMN labels: {dmn_labels}")
print(f"Chosen DAN labels: {dan_labels}")
...
print(f"  DMN (labels {dmn_labels}): {dmn_mask.sum()}")
print(f"  DAN (labels {dan_labels}): {dan_mask.sum()}")
...
for session_dir in session_dirs:
    ...
    for func_path in func_files:
        ...
        # Use the cleaned data directly:
        func_img = nib.load(func_path) 
        func_data = func_img.get_fdata()

        print(f"  Using cleaned data: {os.path.basename(func_path)}")
        ...
        # MAYBE REDUNDANT, AS DATA IS ALREADY CLEANED
        # CHANGE 3: Only apply minimal cleaning (detrend + standardize, NO nuisance regression)
        ...
        # CHANGE 4: Extract DMN/DAN target timeseries (same minimal cleaning)
        dmn_ts_run = func_data[:, dmn_mask].mean(axis=1)
        dan_ts_run = func_data[:, dan_mask].mean(axis=1)
        
        dmn_clean = clean(
            dmn_ts_run.reshape(-1, 1),
            detrend=True,
        ...
        dan_clean = clean(
            dan_ts_run.reshape(-1, 1),
            detrend=True,
            standardize='zscore_sample',
            confounds=None
        ).ravel()

        # CHANGE 5: Add diagnostics
        print(f"  Timeseries stats (fMRIPrep cleaned GLM data):")
        print(f"    FPN1: mean={ts_subnet1_clean.mean():.3f}, std={ts_subnet1_clean.std():.3f}, range=[{ts_subnet1_clean.min():.2f}, {ts_subnet1_clean.max():.2f}]")
        print(f"    FPN2: mean={ts_subnet2_clean.mean():.3f}, std={ts_subnet2_clean.std():.3f}, range=[{ts_subnet2_clean.min():.2f}, {ts_subnet2_clean.max():.2f}]")
        print(f"    DMN: mean={dmn_clean.mean():.3f}, std={dmn_clean.std():.3f}")
        print(f"    DAN: mean={dan_clean.mean():.3f}, std={dan_clean.std():.3f}")

        # ========== 3c) LOAD EVENTS AND SELECT CONDITIONS ==========
        # NEW: use IBC GLM-style preprocessing
        try:
            events = make_paradigm(onset_path, paradigm_id=task)
        except Exception as e:
            print(f"Skipping {run_id}: error in make_paradigm ({e})")
            continue

        # Ensure numeric onset/duration
        ...
        if 'trial_type' not in events.columns:
            print(f"Skipping {run_id}: no trial_type column after make_paradigm")
            continue
        ...
        print(f"  Selected conditions: {sorted(selected_conditions)}")
            ...
            for target_name, target_ts in [('DMN', dmn_clean), ('DAN', dan_clean)]:
                ...
                for condition in selected_conditions:
                    ...
                    if np.allclose(psych_regressor.std(), 0, atol=1e-8):
                        print(f"    Skipping {condition}: constant psychological regressor (onsets={len(onsets)})")
                        continue

                    # Also check if HRF convolution worked:
                    if not np.isfinite(psych_regressor).all():
                        print(f"    Skipping {condition}: NaN in psychological regressor")
                        continue

                    # CREATE PPI REGRESSOR FIRST
                    ppi_regressor = seed_ts * psych_regressor

                    # THEN check variance
                    ppi_variance = np.var(ppi_regressor)
                    if ppi_variance < 1e-6:  # Only skip if essentially constant
                        print(f"    Skipping {condition}: PPI regressor has near-zero variance")
                        continue
