...
def calculate_between_subject_overlap(labels_dict, method_name):
    ...
    return ari_matrix, nmi_matrix, subjects_list

kmeans_ari, kmeans_nmi, subjects_used = calculate_between_subject_overlap(kmeans_labels, "k-means")
infomap_ari, infomap_nmi, _ = calculate_between_subject_overlap(infomap_labels, "infomap")
...
comparison_results = {
    'Metric': ['ARI', 'NMI'],
    'K-means_mean': [kmeans_ari_mean, kmeans_nmi_mean],
    'K-means_std': [kmeans_ari_std, kmeans_nmi_std],
    'Infomap_mean': [infomap_ari_mean, infomap_nmi_mean],
    'Infomap_std': [infomap_ari_std, infomap_nmi_std],
    'Difference': [kmeans_ari_mean - infomap_ari_mean, kmeans_nmi_mean - infomap_nmi_mean]
}
...
if kmeans_ari_mean > infomap_ari_mean:
    print(f"\n→ K-means shows HIGHER between-subject consistency (ARI: {kmeans_ari_mean:.3f} vs {infomap_ari_mean:.3f})")
else:
    print(f"\n→ Infomap shows HIGHER between-subject consistency (ARI: {infomap_ari_mean:.3f} vs {kmeans_ari_mean:.3f})")
